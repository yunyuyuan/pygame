...
class Window(object):
    def __init__(self, surface, pos, window_mod, alert_mod, texts=['确定', '取消'], rotate_speed=10, text_info=['', [0, 0], ['kaiti', 20, 0]]):
        ...
        self.texts = texts
        # 窗口背景
        self.window = pygame.image.load('gametool/' + window_mod + '.png').convert_alpha()
        ...
        for i in range(len(self.texts)):
            exec('self.button%d = Button(self.screen, self.texts[i], [surface.get_width()/2-self.window_width//2+(self.window_width*(i+1)/%d), surface.get_height()/2-self.window_height//2+self.window_height-100], multiple=0.7, font_pos_alter=[20, 10])' % (i+1, len(self.texts)+1))
        # 是否还存在
        self.life = True
    ...
    def load_alert(self, alert_mod, rotate_speed):
        if alert_mod != '':
            self.alert = pygame.image.load('gametool/' + alert_mod + '.png').convert_alpha()
    ...
    # 添加/删除文字
    def load_text(self, text, pos, text_info=['kaiti', 20, 0]):
        if text != '':
            if isinstance(text, list):
                self.text = []
                self.text_pos = []
                for t in text:
                    i = pygame.font.SysFont(*text_info).render(t, 0, [100, 100, 0])
                    self.text.append(i)
                for p in pos:
                    self.text_pos.append(p)
            else:
                self.text = pygame.font.SysFont(*text_info).render(text, 0, [100, 100, 0])
                self.text_pos = pos
        else:
            self.text = None
    ...
    def blit(self):
        ...
        if self.text:
            if isinstance(self.text, list):
                for t, p in zip(self.text, self.text_pos):
                    self.screen.blit(t, p)
            else:
                self.screen.blit(self.text, self.text_pos)
        # 画出按钮
        for i in range(len(self.texts)):
            exec('self.button%d.blit()' % (i+1))

    # 鼠标在按钮上悬浮
    def hover(self, pos):
        for i in range(len(self.texts)):
            exec('self.button%d.hover(pos)' % (i+1))

    # 鼠标点击
    def click(self, pos):
        for i in range(len(self.texts)):
            is_click = eval('self.button%d.click(pos)' % (i+1))
            if is_click:
                # 完成，退出
                self.life = False
                return self.texts[i]
